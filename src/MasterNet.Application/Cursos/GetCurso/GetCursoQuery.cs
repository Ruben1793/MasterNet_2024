...
public class GetCursoQuery 
{
    ...
    internal class GetCursoQueryHandler : IRequestHandler<GetCursoQueryRequest, Result<CursoResponse>>
    {
        ...
        {
            var curso = await _context.Cursos.Where(x => x.Id == request.Id).Include(x => x.Instructores)
                                        .Include(x => x.Calificaciones).Include(x => x.Precios)
...
public record CursoResponse(Guid? Id, string? Titulo, string? Descripcion, DateTime? FechaPublicacion, 
                            List<InstructorResponse>? Instructores, List<CalificacionResponse>? Calificaciones, 
                            List<PrecioResponse>? Precios, List<PhotoResponse>? Photos)
{
    public CursoResponse() : this(null, null, null, null, null, null, null, null){}
}
