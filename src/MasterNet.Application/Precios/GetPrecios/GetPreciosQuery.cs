...
public class GetPreciosQuery
{
    ...
    internal class GetPreciosQueryHandler : IRequestHandler<GetPreciosQueryRequest, Result<PagedList<PrecioResponse>>>
    {
        ...
        public async Task<Result<PagedList<PrecioResponse>>> Handle(GetPreciosQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                Expression<Func<Precio, object>> selector = request.PreciosRequest!.OrderBy.ToLower() switch
                {
                    "nombre" => x => x.Nombre!,
                ...
                queryable = orderBy ? queryable.OrderBy(selector) : queryable.OrderByDescending(selector);
            }
...
public record PrecioResponse(Guid? Id, string? Nombre, decimal? PrecioActual, decimal? PrecioPromocion)
{
